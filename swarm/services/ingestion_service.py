...
from swarm.models.document import (
    Document,
    DocumentInfo,
    DocumentType
)
...
from swarm.base.base_utils import (
...
from swarm.requests import IngestFilesRequest, UpdateFilesRequest
...
class IngestionService(Service):
    ...
    async def ingest_documents(
        ...
        **kwargs: Any,
        ...
        if duplicate_documents:
            ...
            raise SwarmException(status_code=418, message=warning_message)
    ...
    async def _process_ingestion_results(
        ...
        if ingestion_results["embedding_pipeline_output"]:
            ...
            for doc_id, error in results.items():
                if isinstance(error, SwarmDocumentProcessingError):
                    logger.error(
    ...
    def parse_ingest_files_form_data(
        ...
        try:
            ...
            return IngestFilesRequest(**request_data)
        except json.JSONDecodeError as e:
            raise SwarmException(
                status_code=400, message=f"Invalid JSON in form data: {e}"
            )
        except ValueError as e:
            raise SwarmException(status_code=400, message=str(e))
    ...
    def parse_update_files_form_data(
        ...
        try:
            ...
            return UpdateFilesRequest(**request_data)
        except json.JSONDecodeError as e:
            raise SwarmException(
                status_code=400, message=f"Invalid JSON in form data: {e}"
            )
