...
@pytest.mark.asyncio
async def test_config_loading_required_keys(mock_bad_file):
    with pytest.raises(KeyError):
        SwarmConfig.from_json("config.json")


@pytest.mark.asyncio
async def test_config_loading(mock_file):
    config = SwarmConfig.from_json("config.json")
    assert (
...
def test_swarm_config_serialization(mock_file, mock_redis_client):
    config = SwarmConfig.from_json("config.json")
...
def test_swarm_config_deserialization(mock_file, mock_redis_client):
    config_data = {
        "app": {"max_file_size_in_mb": 128},
        "embedding": {
            "provider": "example_provider",
...
def test_swarm_config_missing_required_key():
    invalid_data = {
        "app": {"max_file_size_in_mb": 128},
        "embedding": {
            "base_model": "model",
